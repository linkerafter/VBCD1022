...
try:
    ...
    class _DummyCurses:
        """Lightweight placeholder to avoid crashes when curses is missing."""
        ...
        @staticmethod
        def init_pair(*_args, **_kwargs) -> None:
            return None

        @staticmethod
        def color_pair(*_args, **_kwargs) -> int:
            return 0

    curses = _DummyCurses()  # type: ignore
    ...
    def play_beep(self, freq: int = 600, duration_ms: int = 80) -> None:
        ...
        try:
            if self.use_winsound:
                ...
                sample_rate = 44100
                t = max(duration_ms / 1000.0, 0.01)
                num_samples = max(1, int(sample_rate * t))
                samples = array(
                    "h",
                    (
                        int(
                ...
                self._simpleaudio.play_buffer(samples.tobytes(), 1, 2, sample_rate)  # type: ignore[attr-defined]
        except Exception:
            self.available = False
...
class ForgeView:
    ...
    ANVIL_FRAMES = [
        [line.replace("§", "\\") for line in frame]
        for frame in _ANVIL_TEMPLATE
    ]
...
# =============================
def fallback_mode(extra_info: Optional[str] = None) -> None:
...
if __name__ == "__main__":
    try:
        if HAS_CURSES:
            ...
            except (curses.error, Exception) as exc:
